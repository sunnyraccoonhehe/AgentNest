...
const Calendar: React.FC<Props> = ({ initialDate, selectedDate, onSelectDate, plans, customHeight, onMonthChange }) => {
	...
	return (
		<div className={styles.calendar} style={{ height: customHeight ?? 'auto' }}>
			<div className={styles.header}>
				<button className={styles.navBtn} onClick={() => handleMonthChange(-1)} aria-label="Назад">
					<IoChevronBack size={20}/>
				</button>
				<div className={styles.title}>{title}</div>
				<button className={styles.navBtn} onClick={() => handleMonthChange(1)} aria-label="Вперёд">
			...
			<div className={styles.legend}>
				...
				<div className={styles.legendRow}>
					<span className={`${styles.legendSquare} ${styles.legendToday}`} />
					<span>- сегодня</span>
				</div>
			</div>
		</div>
